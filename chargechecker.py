...
for file in onlyfiles:
    ...
    for i in np.arange(np.shape(chargearr)[0]):
        ...
        cb=plt.colorbar()
        cb.set_label('Charge (Relative Units)')
        plt.show()
        '''
...
ax1.hist(chargesums,bins=100,label='48x48',alpha=0.2)
ax1.hist(cutsums,bins=100,label='32x32',alpha=0.2)
ax1.set_xlabel('Total Charge (Relative Units)',size='large')
ax1.set_ylabel('Frequency (Counts)',size='large')
...
ax2.hist(chargesums2,bins=100,label='48x48',alpha=0.2)
ax2.hist(cutsums2,bins=100,label='32x32',alpha=0.2)
ax2.set_xlabel('Total Charge (p.e.)',size='large')
ax2.set_ylabel('Frequency (Counts)',size='large')
ax2.set_title('Diffuse Run',size='large')
ax2.semilogy()
ax2.legend(fontsize='large')
plt.tight_layout()
plt.savefig('/home/spencers/Figures/chargehist.png',figsize=(11.69, 8.27), dpi=300)
plt.show()
